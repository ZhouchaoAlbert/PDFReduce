...
CString Util::Path::GetPDFOutPath(CString strPdfInPath, CString strPdfOutFolder)
{
	...
	strPdfFileName = strPdfInPath.Mid(iStart+1, strPdfInPath.GetLength() - 4 - iStart-1);
	CString strPdfOutPath;
	for (UINT32 uIndex = 1; uIndex < 200;uIndex++)
	{
		strPdfOutPath.Format(_T("%s\\%s_%d.pdf"), strPdfOutFolder,strPdfFileName, uIndex);
		if (!Util::Path::IsFileExist(strPdfOutPath))
		{
			break;
		}
	}
	return strPdfOutPath;
}
