...
namespace DuiLib
{
	...
	void CButtonUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
	{
		...
		else if (_tcscmp(pstrName, _T("link")) == 0)
		{	
			if( _tcscmp(pstrValue, _T("true")) == 0)
			{
				m_bLink = true;
	...
	void CButtonUI::PaintText(HDC hDC)
	{
		...
		else
		{
			...
			{
				if((m_uButtonState & UISTATE_HOT) || (m_uButtonState & UISTATE_PUSHED))
				{
					TFontInfo* pFontInfo = m_pManager->GetFontInfo(m_iFont);
					if(pFontInfo != NULL && pFontInfo->bUnderline == false)
					{
						HFONT hFont = m_pManager->GetFont(pFontInfo->sFontName, pFontInfo->iSize, pFontInfo->bBold, true, pFontInfo->bItalic);
						if (hFont == NULL)
						{
							hFont = m_pManager->AddFont(pFontInfo->sFontName, pFontInfo->iSize, pFontInfo->bBold, true, pFontInfo->bItalic);
							if (hFont != NULL)
							{
								HFONT hOld = (HFONT)::SelectObject(hDC, hFont);
								::SetBkMode(hDC, TRANSPARENT);
								::SetTextColor(hDC, RGB(GetBValue(clrColor), GetGValue(clrColor), GetRValue(clrColor)));
								::DrawText(hDC, m_sText, -1, &rc, m_uTextStyle | DT_NOPREFIX);
