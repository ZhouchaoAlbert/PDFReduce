#include "stdafx.h"
#include "UIEdit.h"
#include <windowsx.h>		//zjg 20160223 因为和Cef的CefDOMNode一些函数定义冲突，从UILib.h移到UIBase.cpp
...
namespace DuiLib
{
	...
	CEditWnd::CEditWnd() : m_pOwner(NULL), m_hBkBrush(NULL), m_bInit(false), m_bBtnState(false)
	{
	}
	...
	void CEditWnd::SetBtnState(bool b)
	{
		m_bBtnState = b;
	}
	...
	LRESULT CEditWnd::HandleMessage(UINT uMsg, WPARAM wParam, LPARAM lParam)
	{
		...
		if(m_pOwner && m_pOwner->CanHandleEditWndMsg())
		{
			lRes = m_pOwner->HandleEditWndMsg(uMsg, wParam, lParam, bHandled);
			if(bHandled)
			{
				return lRes;
			}
		...
		{
			m_pOwner->GetManager()->SendNotify(m_pOwner, DUI_MSGTYPE_CHAR, wParam, lParam);
			bHandled = FALSE;
		}
		...
		else if (m_bBtnState && uMsg == WM_SETCURSOR)
		{
	...
	LRESULT CEditWnd::OnKillFocus(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
	{
		...
		if(m_pOwner->GetManager()->GetFocus() == m_pOwner)
		{
			bool bUnfocus = m_pOwner->GetManager()->GetUnfocusPaintWindow();
			m_pOwner->GetManager()->SetUnfocusPaintWindow(true);
			m_pOwner->GetManager()->SetFocus(NULL);
			m_pOwner->GetManager()->SetUnfocusPaintWindow(bUnfocus);
		}
	...
	BOOL CEditWnd::ShowBalloonTip(LPCTSTR lpszText)
	{
	...
	CEditUI::CEditUI() : m_pWindow(NULL), m_uMaxChar(255), m_bReadOnly(false),
		...
		m_dwEditbkColor(0xFFFFFFFF), m_dwEditTextColor(0x00000000), m_iWindowStyls(0), m_dwTipValueColor(0xFFBAC0C5), m_bHandleEditWndMsg(false)
	{
	...
	LPCTSTR CEditUI::GetForeImage()
	{
		return m_sForeImage;
	}

	void CEditUI::SetForeImage(LPCTSTR pStrImage)
	{
	...
	void CEditUI::SetSel(DWORD dwSelection)
	{
	...
	void CEditUI::ShowBalloonTip(LPCTSTR lpszText)
	{
		m_sBalloonTip = lpszText;
	}

	void CEditUI::InvalidateSel(DWORD dwSelection)
	{
		m_bInvalidate = true;
		m_dwSelection = dwSelection;
	}

	bool CEditUI::CanHandleEditWndMsg()
	{
		return m_bHandleEditWndMsg;
	}

	LRESULT CEditUI::HandleEditWndMsg(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
	{
		bHandled = FALSE;
		return 0;
	}
	...
	void CEditUI::SetBtnState(bool b)
	{
		if (m_pWindow)
		{
			m_pWindow->SetBtnState(b);
		}
	}
