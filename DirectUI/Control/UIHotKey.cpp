...
void CHotKeyWnd::Init(CHotKeyUI * pOwner)
{
	...
	do 
	{
		...
		{
			// http://bbs.csdn.net/topics/80051093
			HIMC hHimc;
			// Function for disabling IME
			// Get input context for backup
			hHimc = ImmGetContext(m_hWnd);
			...
			ImmReleaseContext(m_hWnd, hHimc);
		}
		::SetFocus(m_hWnd);
		m_bInit = true;   
	} while (0); 
...
LRESULT CHotKeyWnd::OnEditChanged(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	...
	{
		m_pOwner->m_sText = GetHotKeyName();
		int nLen = m_pOwner->m_sText.GetLength();
		if (nLen == 1){
			m_pOwner->m_wModifiers = 6;
			SetHotKey(m_pOwner->m_wVirtualKeyCode, m_pOwner->m_wModifiers);
			m_pOwner->m_sText = GetHotKeyName();
		}
		else
		{
			if (m_pOwner->m_wVirtualKeyCode >= 33 && m_pOwner->m_wVirtualKeyCode <= 45 && m_pOwner->m_wModifiers == 8)
			{
				m_pOwner->m_wModifiers = 14;
				SetHotKey(m_pOwner->m_wVirtualKeyCode, m_pOwner->m_wModifiers);
				m_pOwner->m_sText = GetHotKeyName();
			}
		}
