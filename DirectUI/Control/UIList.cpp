...
CListUI::CListUI() : m_pCallback(NULL), m_bScrollSelect(false), m_iCurSel(-1), m_iExpandedItem(-1),m_bFastScorll(false)
{
...
void CListUI::DoEvent(TEventUI& event)
{
    ...
    case UIEVENT_SCROLLWHEEL:
        {
			if (m_bFastScorll)
			{
				...
				LONG lDeltaY = 0;
				if(m_totalNum > 0)
					lDeltaY = (LONG)(pow((double)m_leftNum / m_totalNum, 2) * m_delta);
				...
				if
					(m_delta > 100) m_delta = 100;
				else if
					(m_delta < -100) m_delta = -100;

				m_totalNum = (DWORD)sqrt((double)abs(m_delta)) * 5;
				m_leftNum = m_totalNum;
#endif

				m_pManager->SetTimer(this, FASTSCROLL_TIMERID, 20U);
				return;
			}
			else
			{
	...
	case UIEVENT_TIMER:
		{
			if(event.wParam == FASTSCROLL_TIMERID)
			{
				if(m_leftNum > 0)
				{
					--m_leftNum;
					SIZE sz = GetScrollPos();
					LONG lDeltaY = (LONG)(pow((double)m_leftNum / m_totalNum, 2) * m_delta);
				...
				m_totalNum = 0;
				m_leftNum = 0;
				m_pManager->KillTimer(this, FASTSCROLL_TIMERID);
			}
