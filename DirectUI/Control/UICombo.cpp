...
void CComboWnd::Init(CComboUI* pOwner)
{
    ...
    if (m_pOwner) m_pOwner->SetAttribute(_T("dropstate"), _T("true"));
}
    ...
    if( uMsg == WM_CREATE ) {
	   ...
	   // m_pLayout->SetInset(CDuiRect(2, 2, 2, 2));
		...
		if (m_hWnd != (HWND)wParam) {
			...
			m_pOwner->SetPos(m_pOwner->GetPos());
			m_pOwner->SetFocus();
            m_pOwner->SetAttribute(_T("dropstate"), _T("false"));
...
void CComboWnd::EnsureWheel(int zDelta)
{
	int nCount = m_pLayout->GetCount();
	if (nCount <= 0) return;
	RECT rcItem = m_pLayout->GetItemAt(0)->GetPos();
	int dx = rcItem.bottom - rcItem.top;
	if (zDelta < 0)
		Scroll(0, dx);
	else
		Scroll(0, -dx);
...
void CComboUI::PaintText(HDC hDC)
{
	...
	{
		TListInfoUI* pInfo = GetListInfo();
		if(pInfo != NULL)
		{
			RECT rcText__ = rcText;
			rcText__.left += pInfo->rcTextPadding.left;
			rcText__.right -= pInfo->rcTextPadding.right;
			rcText__.top += pInfo->rcTextPadding.top;
			rcText__.bottom -= pInfo->rcTextPadding.bottom;
			CRenderEngine::DrawText(hDC, m_pManager, rcText__, m_sText, pInfo->dwSelectedTextColor, pInfo->nFont, DT_SINGLELINE | pInfo->uTextStyle);
		}
	}
}
...
int CComboUI::GetArrowWidth()
{
    return m_nArrowWidth;
}

void CComboUI::SetNormalArrow(LPCTSTR pStrImage)
{
    m_sNormalArrow = pStrImage;
    Invalidate();
}

void CComboUI::SetHotArrow(LPCTSTR pStrImage)
{
    m_sHotArrow = pStrImage;
    Invalidate();
}

void CComboUI::SetPushedArrow(LPCTSTR pStrImage)
{
    m_sPushedArrow = pStrImage;
    Invalidate();
}

void CComboUI::SetDisabledArrow(LPCTSTR pStrImage)
{
    m_sDisabledArrow = pStrImage;
    Invalidate();
}

void CComboUI::SetDropArrow(LPCTSTR pStrImage)
{
    m_sDropArrow = pStrImage;
    Invalidate();
}

void CComboUI::PaintStatusArrowImage(HDC hDC)
{
    CDuiString sModify;
    CSize size(m_rcItem);
...
void CComboUI::SetDropState(bool bState)
{
    m_bDropState = bState;
    Invalidate();
}
