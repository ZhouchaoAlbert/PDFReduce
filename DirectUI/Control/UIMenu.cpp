...
void CMenuWnd::ResizeSubMenu()
{
	...
	MoveWindow(m_hWnd, rc.left - 3, rc.top + 100, rc.right - rc.left, rc.bottom - rc.top + m_pLayout->GetInset().top + m_pLayout->GetInset().bottom, FALSE);
...
void CMenuElementUI::DrawItemText(HDC hDC, const RECT& rcItem)
{
    ...
    if ((m_uButtonState & UISTATE_HOT) != 0) {
        ...
        if (!m_sHotText.IsEmpty()) sText = m_sHotText;
    }
    if (IsSelected()) {
		...
		if( IsEnabled() ){
			...
			else
			{
				...
				{ //redrain 这样修改的
					...
					//PostMessage(CMenuWnd::GetGlobalContextMenuObserver().GetManager()->GetPaintWindow(), WM_MENUCLICK, (WPARAM)RegisterWindowMessage(GetName()), (LPARAM)(GetChecked() == TRUE));
                    PostMessage(CMenuWnd::GetGlobalContextMenuObserver().GetManager()->GetPaintWindow(), WM_MENUCLICK, (WPARAM)pMenuCmd, NULL);
				}
				ContextMenuParam param;
				param.hWnd = m_pManager->GetPaintWindow();
				param.wParam = 1;
...
void CMenuElementUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
    ...
    else if (_tcscmp(pstrName, _T("hottext")) == 0){
        m_sHotText = pstrValue;
    }
	else
