...
BOOL CTxtWinHost::TxSetScrollPos (INT fnBar, INT nPos, BOOL fRedraw)
{
    ...
    CScrollBarUI* pHorizontalScrollBar = m_re->GetHorizontalScrollBar();
    if( fnBar == SB_VERT && pVerticalScrollBar ) {
// 		int nRange = pVerticalScrollBar->GetScrollRange();
// 		if (nPos > nRange){
// 			GetTextServices()->TxSendMessage(WM_VSCROLL, SB_BOTTOM, 0L, 0);
// 		}
        pVerticalScrollBar->SetScrollPos(nPos);
    }
    else if( fnBar == SB_HORZ && pHorizontalScrollBar ) {
// 		int nRange = pHorizontalScrollBar->GetScrollRange();
// 		if (nPos > nRange){
// 			GetTextServices()->TxSendMessage(WM_VSCROLL, SB_BOTTOM, 0L, 0);
// 		}
...
{
    bool bReadOnly = m_re->IsReadOnly();
    if (bReadOnly)
    {
...
void CRichEditUI::SetLimitText(int iChars)
{
	...
	// modify 2017-7-19 by houbin 修复设置richedit最大文本长度无效
    m_iLimitText = iChars;
	TxSendMessage(EM_EXLIMITTEXT, 0, iChars, &lResult);
#if 0
	if (m_pTwh) {
...
bool CRichEditUI::SetAutoURLDetect(bool bAutoDetect)
{
    LRESULT lResult;
	if (bAutoDetect)
		TxSendMessage(EM_AUTOURLDETECT, 1, 0, &lResult);
	else
		TxSendMessage(EM_AUTOURLDETECT, 0, 0, &lResult);
...
// 		}
	...
	if( event.Type == UIEVENT_KILLFOCUS )  {
        ...
        if (event.wParam == TIMERID_DELAYED)
        {
            if (m_bAdustScroll == SB_TOP){
                TxSendMessage(WM_VSCROLL, SB_TOP, 0L, 0);
            }
            else if (m_bAdustScroll == SB_BOTTOM){
                TxSendMessage(WM_VSCROLL, SB_BOTTOM, 0L, 0);
            }
            GetManager()->KillTimer(this, TIMERID_DELAYED);
            return;
        }
        if( m_pTwh ) {
...
void CRichEditUI::SetPos(RECT rc)
{
	...
	if( m_pTwh ) {
		...
		if (m_pVerticalScrollBar && m_pVerticalScrollBar->IsVisible())
		{
			int nVPos1 = m_pVerticalScrollBar->GetScrollPos();
			int nVRange1 = m_pVerticalScrollBar->GetScrollRange();
			if (nVPos1 && nVPos1 == nVRange1){
				m_bAdustScroll = SB_BOTTOM;
			}
		}
        SIZE sRange = GetScrollRange();
		SIZE sScrollPos = GetScrollPos();
		m_pTwh->SetClientRect(&rcRich);
        if (sRange.cy == sScrollPos.cy)
        {
            m_bAdustScroll = SB_BOTTOM;
        }
        else
	...
	if (m_bAdustScroll == SB_TOP){
        if (!::EqualRect(&rcOld, &m_rcItem))
            GetManager()->SetTimer(this, TIMERID_DELAYED, 20);
        else
            TxSendMessage(WM_VSCROLL, SB_TOP, 0L, 0);
	}
	else if (m_bAdustScroll == SB_BOTTOM){
...
void CRichEditUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
	...
	else if (_tcscmp(pstrName, _T("tip")) == 0) m_sTip = pstrValue;
...
LRESULT CRichEditUI::MessageHandler(UINT uMsg, WPARAM wParam, LPARAM lParam, bool& bHandled)
{
    ...
    if (!IsWantTab() && (uMsg == WM_CHAR || uMsg == WM_IME_CHAR) && wParam == VK_TAB) return 0;

	if (uMsg == WM_IME_COMPOSITION)
	{
        /*解决新消息来的时候输入法输入框跳到其它地方问题*/
        CHARRANGE cr = { 0 };
        GetSel(cr);
        SetSel(cr);
    ...
    if( (uMsg >= WM_MOUSEFIRST && uMsg <= WM_MOUSELAST) || uMsg == WM_SETCURSOR ) {
        ...
        if( dwHitResult == HITRESULT_OUTSIDE ) {
			...
			if (uMsg == WM_MOUSEWHEEL) {
				...
				return 0;//屏蔽richedit滚动放大缩小功能
			}
            if( ::PtInRect(&rc, pt) && !GetManager()->IsCaptured() ) dwHitResult = HITRESULT_HIT;
        }
        //添加外部响应用
        if (uMsg == WM_SETCURSOR) OnSetCursor(wParam, lParam, bHandled);
        if( dwHitResult != HITRESULT_HIT ) return 0;
        if( uMsg == WM_SETCURSOR ) bWasHandled = false;
		else if (uMsg == WM_LBUTTONDOWN || uMsg == WM_LBUTTONDBLCLK || uMsg == WM_RBUTTONDOWN){
			::SetFocus(GetManager()->GetPaintWindow());//修改bug 子窗口时候点击没有设置窗口焦点
			bool bUnfocus = m_pManager->GetUnfocusPaintWindow();
			m_pManager->SetUnfocusPaintWindow(true);
			SetFocus();
			m_pManager->SetUnfocusPaintWindow(bUnfocus);
        }
...
// 			break;
    ...
    {
        OnLButtonDown(wParam, lParam, bHandled);
        if (bHandled) return 0;
    }
    else if (WM_LBUTTONDBLCLK == uMsg)
    {
        OnLDButtonDown(wParam, lParam, bHandled);
        if (bHandled) return 0;
    }
    else if (WM_RBUTTONUP == uMsg)
    {
        OnRButtonUp(wParam, lParam, bHandled);
        if (bHandled) return 0;
    }
    else if (WM_CONTEXTMENU == uMsg)
    {
        OnContextMenu(wParam, lParam, bHandled);
        if (bHandled) return 0;
    }
...
LRESULT CRichEditUI::OnSetCursor(WPARAM wParam, LPARAM lParam, bool& bHandled)
{
    bHandled = false;
    return E_FAIL;
}

LRESULT CRichEditUI::OnLButtonDown(WPARAM wParam, LPARAM lParam, bool& bHandled)
{
    bHandled = false;
    return E_FAIL;
}

LRESULT CRichEditUI::OnLDButtonDown(WPARAM wParam, LPARAM lParam, bool& bHandled)
{
    bHandled = false;
	return E_FAIL;
}

LRESULT CRichEditUI::OnMouseMove(WPARAM wParam, LPARAM lParam, bool& bHandled)
{
    bHandled = false;
	return 0L;
}

LRESULT CRichEditUI::OnRButtonUp(WPARAM wParam, LPARAM lParam, bool& bHandled)
{
    bHandled = false;
    return 0L;
}

LRESULT CRichEditUI::OnContextMenu(WPARAM wParam, LPARAM lParam, bool& bHandled)
{
    bHandled = false;
    return 0L;
}

LRESULT CRichEditUI::OnChar(WPARAM wParam, LPARAM lParam, bool& bHandled)
{
    bHandled = false;
    return 0L;
}
...
IRichEditOle * CRichEditUI::GetIRichEditOle()
{
	...
	HRESULT hr ;
	TxSendMessage( EM_GETOLEINTERFACE, 0, (LPARAM)&pRichItem, &hr);
	return pRichItem;
}
...
{
	HRESULT hr = S_OK;
	TxSendMessage(EM_GETFIRSTVISIBLELINE, 0, 0, &hr);
	return (int)hr;
}
...
void CRichEditUI::GetVisibleCharRange(CHARRANGE &chr)
{
    ...
    POINT  ptRightBottom = { m_rcItem.right, m_rcItem.bottom };
    chr.cpMax = CharFromPos(ptRightBottom);
}

//获取可见区域的开始结束索引（-1代表没有找到）
void CRichEditUI::GetVisibleOlePos(int & nFirstVisibleOle, int & nLastVisibleOle)
{
    HRESULT hr = S_OK;
    IRichEditOle *	spRichEditOle = NULL;

    spRichEditOle = GetIRichEditOle();
    //获得可见字符范围
   
	int iFirstLine = GetFirstVisibleLine();
    RECT rcView = m_rcItem;
    POINT pt = { rcView.right + 1, rcView.bottom - 2 };
    long cpFirst = LineIndex(iFirstLine);
    long cpLast = CharFromPos(pt);

    //采用两分法查找在可见范围中的OLE对象
    int nCount = spRichEditOle->GetObjectCount();
    nFirstVisibleOle = FindFirstOleInrange(spRichEditOle, 0, nCount, cpFirst, cpLast);
    if (nFirstVisibleOle != -1)
        nLastVisibleOle = FindLastOleInrange(spRichEditOle, nFirstVisibleOle, nCount, cpFirst, cpLast);
    else
        nLastVisibleOle = -1;
    spRichEditOle->Release();
}

//获取ole位置
long CRichEditUI::GetOleCP(IRichEditOle *pOle, int iOle)
{
    REOBJECT reobj = { 0 };
    reobj.cbStruct = sizeof(REOBJECT);
    HRESULT hr = pOle->GetObject(iOle, &reobj, REO_GETOBJ_NO_INTERFACES);
    if (SUCCEEDED(hr))
    {
        if (reobj.poleobj)
...
int CRichEditUI::FindFirstOleInrange(IRichEditOle *pOle, int iBegin, int iEnd, int cpMin, int cpMax)
{
...
int CRichEditUI::FindLastOleInrange(IRichEditOle *pOle, int iBegin, int iEnd, int cpMin, int cpMax)
{
    if (iBegin == iEnd) return -1;
...
BOOL CRichEditUI::IsCanPaste()
{
    ...
    if (::OpenClipboard(GetManager()->GetPaintWindow()))
	{
		...
		if (uFormat > 0)
		{
			HGLOBAL hMemory = ::GetClipboardData(uFormat);
			if (NULL != hMemory){
				bFlag = TRUE;
			}
		}
...
//拖拽事件注册和反注册
void CRichEditUI::EnableDragDrop(BOOL bEnable)
{
    if (m_pManager == NULL)
        return;
    ...
    if (bEnable)
    {
        CRicheditDropTarget *pDropTarget = new CRicheditDropTarget(m_pTwh->GetTextServices());
        ::RegisterDragDrop(m_pManager->GetPaintWindow(), pDropTarget);
        pDropTarget->Release();
    }
    else
    {
...
RECT CRichEditUI::GetCaretRect()
{
    ...
    CComPtr<ITextDocument> spTextDoc;

    //默认位置
    ::GetCaretPos(&pt);
    rcItem.left = rcItem.right = pt.x;
    rcItem.top = rcItem.bottom = pt.y;
#if 0
    spRichEditOle.Attach(GetIRichEditOle());
    HRESULT hr = spRichEditOle->QueryInterface(__uuidof(ITextDocument), (void**)&spTextDoc);
    if (FAILED(hr))
    {
        return rcItem;
    }
    ...
    CHARRANGE chr = { 0 };
    GetSel(chr);
    hr = spTextDoc->Range(chr.cpMin, chr.cpMax, &spRangeLine);
    if (FAILED(hr))
    {
        return rcItem;
    }
...
#define _tomAllowOffClient  512  //Allow points outside of the client area.
    ...
    hr = spRangeLine->GetPoint(lTypeTopLeft, &ptStart.x, &ptStart.y);
    ASSERT(SUCCEEDED(hr));
    hr = spRangeLine->GetPoint(lTypeRightBottom, &ptEnd.x, &ptEnd.y);
    ASSERT(SUCCEEDED(hr));
    ::SetRect(&rcItem, ptStart.x, ptStart.y, ptEnd.x, ptEnd.y);
#endif
    return rcItem;
}
