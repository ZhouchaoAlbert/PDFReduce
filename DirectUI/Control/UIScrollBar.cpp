...
namespace DuiLib
{
	...
	void CScrollBarUI::SetMinThumbSize(int nThumbSize)
	{
		m_nMinThumbSize = nThumbSize;
		Invalidate();
	}

	int CScrollBarUI::GetMinThumbSize()
	{
		...
		if( m_bHorizontal ) {
			...
			if( cx > m_cxyFixed.cy ) {
				...
				if( m_nRange > 0 ) {
					...
					if (cxThumb < m_nMinThumbSize) cxThumb = m_nMinThumbSize;
			...
			if( cy > m_cxyFixed.cx ) {
				...
				if( m_nRange > 0 ) {
					...
					if (cyThumb < m_nMinThumbSize) cyThumb = m_nMinThumbSize;
	...
	void CScrollBarUI::DoEvent(TEventUI& event)
	{
		...
		if( event.Type == UIEVENT_TIMER && event.wParam == DEFAULT_TIMERID )
		{
			...
			if( (m_uThumbState & UISTATE_CAPTURED) != 0 ) {
				if( !m_bHorizontal ) {
					if (m_pOwner != NULL) {
						m_pOwner->SetScrollPos(CSize(m_pOwner->GetScrollPos().cx, \
							m_nLastScrollPos + m_nLastScrollOffset));
						m_pOwner->HandAdustScroll();
						int nCurPos = m_nLastScrollPos + m_nLastScrollOffset;
						if (m_nLastScrollPos < nCurPos){
							m_pOwner->OnScrollDown();
						}
						else if (m_nLastScrollPos > nCurPos){
							m_pOwner->OnScrollUp();
						}
					}
					...
					if (m_pOwner != NULL) {
						m_pOwner->SetScrollPos(CSize(m_nLastScrollPos + m_nLastScrollOffset, \
			...
			if( m_pManager != NULL && m_pOwner == NULL ) m_pManager->SendNotify(this, DUI_MSGTYPE_SCROLL);
			//新加代码  让RichEdit能够收到 鼠标按下滚动条 上下 滚动的消息=========================
			if (m_pManager != NULL && m_pOwner != NULL)
			{
				CDuiString strClassName = m_pOwner->GetClass();
				if (strClassName == _T("ImRichEditUI"))
				{
					m_pManager->SendNotify(m_pOwner, DUI_MSGTYPE_SCROLL);     //是EC的控件的话接收这个消息
				}
			}
			//======================================
			return;
	...
	void CScrollBarUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
	{
		...
		else if (_tcscmp(pstrName, _T("minthumbsize")) == 0) SetMinThumbSize(_ttoi(pstrValue));
