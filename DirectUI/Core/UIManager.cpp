...
CPaintManagerUI::~CPaintManagerUI()
{
	...
	if (m_hUpdateRectPen != NULL) DeletePen(m_hUpdateRectPen);
...
HRESULT CPaintManagerUI::OnDropEnter(IDataObject *pDataObj, DWORD grfKeyState, POINTL pt, DWORD *pdwEffect)
{
	m_pCurDataObject = pDataObj;
	POINT ppt = { pt.x, pt.y };
	::ScreenToClient(m_hWndPaint, &ppt);
	CControlUI* pHover = FindControl(ppt);
	if (pHover == NULL)
	{
	...
	pHover->OnDropEnter(pDataObj, grfKeyState, ppt, pdwEffect);
	m_pEventDrop = pHover;
	return	S_OK;
}
HRESULT CPaintManagerUI::OnDropOver(DWORD grfKeyState, POINTL pt, DWORD *pdwEffect)
{
	POINT ppt = { pt.x, pt.y };
	::ScreenToClient(m_hWndPaint, &ppt);
	m_ptLastMousePos = ppt;
	CControlUI* pNewHover = FindControl(ppt);
	if (pNewHover == NULL)
	{
		*pdwEffect = DROPEFFECT_NONE;
		return S_OK;
	...
	if (pNewHover != m_pEventDrop && m_pEventDrop != NULL) {
		m_pEventDrop->OnDropLeave();
		m_pEventDrop = NULL;
	}

	if (pNewHover != m_pEventDrop && pNewHover != NULL) {
		pNewHover->OnDropEnter(m_pCurDataObject, grfKeyState, ppt, pdwEffect);
		m_pEventDrop = pNewHover;
	}

	if (pNewHover != NULL) {
		pNewHover->OnDropOver(grfKeyState, ppt, pdwEffect);
	}
	return S_OK;
}
HRESULT CPaintManagerUI::OnDropLeave()
{
...
HRESULT CPaintManagerUI::OnDrop(IDataObject *pDataObj, DWORD grfKeyState, POINTL pt, DWORD *pdwEffect)
{
	POINT ppt = { pt.x, pt.y };
	::ScreenToClient(m_hWndPaint, &ppt);
	if (m_pEventDrop != NULL) {

		m_pEventDrop->OnDrop(pDataObj, grfKeyState, ppt, pdwEffect);
	}
	else
	{
		*pdwEffect = DROPEFFECT_NONE;
		   ...
		   if (m_anim.IsAnimating() || m_anim.IsJobScheduled()){
			   ...
			   else
			   {
				   ...
				   if (m_bOffscreenPaint)
				   {
					   ...
					   if (m_bUseSelfPaint)
					   {
							::SendMessage(m_hWndPaint, WM_SELFPAINT, (WPARAM)m_hDcOffscreen, 0);
					   }
...
void CPaintManagerUI::SetFocusNeeded(bool b)
{
	m_bFocusNeeded = b;
}
...
{
	bool bAddRef = false;
	HFONT hFont_ = TheDuiResMgr.AddFont(bAddRef, pStrFontName, m_hWndPaint, nSize, bBold, bUnderline, bItalic, m_hDcPaint);

	//可以加个单个窗口中去重
	if (hFont_ != NULL && bAddRef)
	{
		if(!m_aCustomFonts.Add(hFont_))
		{
			ASSERT(FALSE);
			return NULL;
		}
...
{
	HFONT hFont = GetFont(pStrFontName, nSize, bBold, bUnderline, bItalic);

	if (hFont != NULL)
	{
		for(int it = 0; it < m_aCustomFonts.GetSize(); it++)
		{
			HFONT hFontTemp = static_cast<HFONT>(m_aCustomFonts[it]);
			if (hFont == hFontTemp)
			{
				return it;
			}
...
{
    return TheDuiResMgr.RemoveFont(m_hWndPaint,hFont);
}
...
bool CPaintManagerUI::RemoveFontAt(int index)
{
	...
	bool bRet = m_aCustomFonts.Remove(index);
	bRet = TheDuiResMgr.RemoveFont(m_hWndPaint, hFont);
...
{
	int ii = m_aCustomFonts.GetSize();
    for( int i = 0; i < m_aCustomFonts.GetSize(); i++ )
	 {
		HFONT hFont = static_cast<HFONT>(m_aCustomFonts[i]);
		bool bRet = TheDuiResMgr.RemoveFont(m_hWndPaint, hFont);
    }
...
TFontInfo* CPaintManagerUI::GetFontInfo(int index)
{
	...
	TFontInfo* pFontInfo = TheDuiResMgr.GetFontInfo(hFont);

    if(pFontInfo != NULL && pFontInfo->tm.tmHeight == 0 )
	 {
        HFONT hOldFont = (HFONT) ::SelectObject(m_hDcPaint, pFontInfo->hFont);
        ::GetTextMetrics(m_hDcPaint, &pFontInfo->tm);
        ::SelectObject(m_hDcPaint, hOldFont);
    }
	ASSERT(pFontInfo != NULL);
...
#else
	return TheDuiResMgr.GetImage(bitmap);
#endif
}
...
#else
	bool bAddRef = false;
	const TImageInfo* pInfo = TheDuiResMgr.AddImage(bAddRef, m_hWndPaint, bitmap, type, mask);
	if(pInfo != NULL && bAddRef)
	{
		m_aImageInfo.Add((LPVOID)pInfo);
	}
...
#else
	bool bAddRef = false;
	const TImageInfo* pInfo = TheDuiResMgr.AddImage(bAddRef, m_hWndPaint, bitmap, hBitmap, iWidth, iHeight, bAlpha);
	if (pInfo != NULL && bAddRef)
	{
		m_aImageInfo.Add((LPVOID)pInfo);
	}
...
#else
	const TImageInfo* pInfo = TheDuiResMgr.GetImage(bitmap);

	for(int i = 0; i < m_aImageInfo.GetSize(); i++)
	{
		const TImageInfo* pTemp = static_cast<TImageInfo*>(m_aImageInfo[i]);

		if(pTemp != NULL)
		{
			if(pInfo == pTemp)
			{
				m_aImageInfo.Remove(i);
				break;
			}
		}
	}

	return TheDuiResMgr.RemoveImage(m_hWndPaint, bitmap);
#endif
}
...
void CPaintManagerUI::RemoveAllImages()
{
	...
	m_mImageHash.RemoveAll();
	  ...
	  for( int i = 0; i < m_aImageInfo.GetSize(); i++ )
	  {
			...
			if(pTemp != NULL)
			{
				TheDuiResMgr.RemoveImage(m_hWndPaint, pTemp->sPath);
			}
	  }
#endif
}
...
bool CPaintManagerUI::GetUnfocusPaintWindow()
{
	return m_bUnfocusPaintWindow;
}

void CPaintManagerUI::SetSelfPaint(bool b)
{
	m_bUseSelfPaint = b;
}

void CPaintManagerUI::SetEventClick(CControlUI *pEventClick)
{
    m_pEventClick = pEventClick;
}

CControlUI * CPaintManagerUI::GetEventClick()
{
