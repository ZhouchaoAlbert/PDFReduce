...
void CControlUI::OnDropEnter(IDataObject *pDataObj, DWORD grfKeyState, POINT pt, DWORD *pdwEffect)
{
	if (m_bDragEnabled)
	{
	...
	else
	{
		*pdwEffect = DROPEFFECT_NONE;
		if (m_pParent != NULL)m_pParent->OnDropEnter(pDataObj, grfKeyState, pt, pdwEffect);
	}
}
void CControlUI::OnDropOver(DWORD grfKeyState, POINT pt, DWORD *pdwEffect)
{
	if (m_bDragEnabled)
	{
		*pdwEffect = DROPEFFECT_MOVE;
	...
	{
		*pdwEffect = DROPEFFECT_NONE;
		if (m_pParent != NULL)m_pParent->OnDropOver(grfKeyState, pt, pdwEffect);
	}
}
void CControlUI::OnDropLeave()
{
	if (m_bDragEnabled)
	{
	}else if (m_pParent != NULL)
		m_pParent->OnDropLeave();
}
...
void CControlUI::SetDropEnabled(bool bDragEnabled)
{
