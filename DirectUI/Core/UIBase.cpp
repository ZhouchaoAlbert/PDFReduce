...
CStdPtrArray CWindowWnd::m_aPreMsgReceiver;
...
UINT CWindowWnd::ShowModal()
{
    ...
    MSG msg = { 0 };
	BOOL bRet = FALSE;
	while (::IsWindow(m_hWnd) && (bRet = ::GetMessage(&msg, NULL, 0, 0))) {
		...
		if (msg.message == WM_CLOSE && msg.hwnd == hOldWnd) break;
		if (msg.message == WM_CLOSE && msg.hwnd == hWndParent) break;
...
HICON CWindowWnd::SetIcon(HICON hIcon, BOOL bBigIcon)
{
...
LRESULT CALLBACK CWindowWnd::__WndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    if( uMsg == WM_NCCREATE ) {
		...
		if(m_aPreMsgReceiver.GetSize() > 0)
		{
			for(int i = 0; i < m_aPreMsgReceiver.GetSize(); i++)
			{
				IPreMsgReceiver* pReceiver = static_cast<IPreMsgReceiver*>(m_aPreMsgReceiver[i]);
				if(pReceiver != NULL)
				{
					if(pReceiver->PreTranslateMsg(hWnd, uMsg, wParam, lParam))
						return 0;;
				}
			}
		}
...
IPreMsgReceiver::IPreMsgReceiver()
{
	CWindowWnd::m_aPreMsgReceiver.Add(this);
}

IPreMsgReceiver::~IPreMsgReceiver()
{
	CWindowWnd::m_aPreMsgReceiver.Remove(CWindowWnd::m_aPreMsgReceiver.Find(this));
}
