...
namespace DuiLib
{
			...
			if( static_cast<CControlUI*>(m_items[it]) == pControl ) {
				...
				if( m_bAutoDestroy ) {
                    if (m_bDelayedDestroy && m_pManager) {
						...
						if (m_pManager && m_pManager->GetEventClick() == pControl){
                            m_pManager->SetEventClick(NULL);
                        }
                        delete pControl;
                    }
				}
	...
	{
		if( m_pVerticalScrollBar && m_pVerticalScrollBar->IsVisible() ) m_pVerticalScrollBar->SetScrollRange(sz.cy);
		if( m_pHorizontalScrollBar && m_pHorizontalScrollBar->IsVisible() ) m_pHorizontalScrollBar->SetScrollRange(sz.cx);
	}	
